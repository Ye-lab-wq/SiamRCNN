...
def center_loss(embedding, label, num_classes, alpha=0.1, scope="center_loss"):
    ...
    centers = tf.compat.v1.get_variable('centers', [num_classes, nrof_features], dtype=tf.float32,
...
class EmbeddingModel(ModelDesc):
    ...
    def optimizer(self):
        lr = tf.compat.v1.get_variable('learning_rate', initializer=1e-4, trainable=False)
